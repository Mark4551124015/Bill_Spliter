...
def check_bill(bill_path, patrons):
    ...
    for line in bill.iterrows():
        ...
        if len(members)==1 and members[0] == 'All':
            for mem in patrons:
                if mem == 'Qiu': continue
                patrons[mem].add(item, quantity/(len(patrons)-1))
        elif len(members)==1 and members[0] == 'TrueAll':
            for mem in patrons:
